...
class Controller(Node):
    ...
    def __init__(self):
        ...
        # ================= NEW: LINE FOLLOWING PARAMETERS =================
        self.declare_parameter('kp_line', 0.5)
        self.declare_parameter('ki_line', 0.0)
        self.declare_parameter('kd_line', 0.05)
        self.declare_parameter('v_line_base', 0.10)
        self.declare_parameter('speed_curve_factor', 0.5)
        self.declare_parameter('line_lost_timeout_s', 0.4)
        ...
        # ================= NEW: LINE FOLLOWING PID =================
        self.pid_line = PID(
            float(self.get_parameter('kp_line').value),
            float(self.get_parameter('ki_line').value),
            float(self.get_parameter('kd_line').value),
            integral_max,
            deriv_alpha
        )
        ...
        # ================= NEW: LINE FOLLOWING STATE =================
        self.line_error = 0.0
        self.line_visible = False
        self.line_lost_since = None

        self.v_line_base = float(self.get_parameter('v_line_base').value)
        self.speed_curve_factor = float(
            self.get_parameter('speed_curve_factor').value)
        self.line_lost_timeout = float(
            self.get_parameter('line_lost_timeout_s').value)

        # In line_following mode the FSM does not transition through a
        # ROTATE -> MOVE -> HOLD cycle driven by goals; the robot just
        # follows the line continuously. Bootstrap out of IDLE so the
        # line_following branch can actually run.
        ...
        # ================= NEW: LINE FOLLOWING SUBSCRIPTIONS =================
        # Only subscribed when actually needed: avoids spinning up an
        # extra topic when running in sequential / simultaneous mode.
        if self.mode == 'line_following':
            line_err_topic = self.get_parameter('line_error_topic').value
            line_det_topic = self.get_parameter('line_detected_topic').value
            self.create_subscription(
                Float32, line_err_topic, self._cb_line_error, 10)
            self.create_subscription(
                Bool, line_det_topic, self._cb_line_detected, 10)
        ...
        # Updated logger to recognize line_following as a valid mode
        # and print the relevant parameters for whichever mode is active.
        if self.mode == 'line_following':
            self.get_logger().info(
                f'Controller up | mode=line_following | sem={sem_str} | '
                f'kp_line={float(self.get_parameter("kp_line").value):.2f} | '
                f'v_line_base={self.v_line_base:.2f} | '
    ...
    # ================= NEW: LINE FOLLOWING CALLBACKS =================

    def _cb_line_error(self, msg: Float32):
    ...
    def _step(self):
        ...
        # ================= LINE FOLLOWING MODE =================
        # New top-level branch. Self-contained: handles its own IDLE
        # check, runs its own line-following logic, applies semaphore
        # override and slew rate, publishes /cmd_vel, logs, and returns
        # BEFORE reaching the sequential/simultaneous code below.
        if self.mode == 'line_following':
            ...
            # Line following main logic
            if self.line_visible:
                # Reset lost timer
                self.line_lost_since = None

                # Angular PID on normalized pixel error
                w_cmd = self.pid_line.update(self.line_error, dt)

                # Linear speed: reduce in curves proportional to error magnitude
                v_cmd = self.v_line_base * (
                    1.0 - self.speed_curve_factor * min(abs(self.line_error), 1.0)
                )
                ...
                if self.line_lost_since is None:
                    self.line_lost_since = now_wall

                elapsed_lost = now_wall - self.line_lost_since

                if elapsed_lost < self.line_lost_timeout:
                    # Grace period: hold last angular command, reduce speed
                    w_cmd = self.last_w_pub
                    v_cmd = self.v_line_base * 0.5
                else:
                    # Full stop
                    if elapsed_lost < self.line_lost_timeout + 0.1:
                        self.get_logger().warn('Line lost -- full stop')
                    self.pid_line.reset()
                    v_cmd = 0.0
                    w_cmd = 0.0

            # Save pre-semaphore values for logging
            ...
            # Publish with slew rate
            v_out, w_out = self._publish_cmd(v_cmd, w_cmd, dt)

            # Log: use line_error in both dist and ang_error columns so
            # offline analysis scripts have it in a known field. The
            # CSV columns 'error_pos' and 'error_ang' will represent
            # the line error magnitude and signed line error in this mode.
            self._maybe_log(
                v_cmd, w_cmd, v_out, w_out,
                dist=abs(self.line_error),
                ang_error=self.line_error
            )
            return

        # ================= SEQUENTIAL / SIMULTANEOUS PATH =================
        ...
        # ================= MODO SIMULTANEO =================
            ...
            # Unknown mode -- bail safely without publishing motion
            return
